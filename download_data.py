'''Uncomment the desired download in downloadCSV function.'''
import urllib.parse
import datetime
        ...
        for row in csv_dict_reader:
            if (timeRange != None): #if time range is specified
                if (int(row['date']) <= int(timeRange[1])) and (int(row['date']) >= int(timeRange[0])): #download from earliest to latest date
                    ...
                    #dl = tiffs(output+"VIR_NOAA_TRUE_"+convertDate(row['date'])+'.tif','VIIRS_NOAA20_CorrectedReflectance_TrueColor',convertDate(row['date']),(float(row[coord1]),float(row[coord2])),resolution)
                    #dl = tiffs(output+"VIR_NOAA_M11I2I1_"+convertDate(row['date'])+'.tif','VIIRS_NOAA20_CorrectedReflectance_BandsM11-I2-I1',convertDate(row['date']),(float(row[coord1]),float(row[coord2])))
