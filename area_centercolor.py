...
def ice_olate(directory,display=False,):
    """Isolate iceberg from its surroundings. Filters out images with less than 25% white pixels and more than 90% white pixels.
        ...
        Args:
            ...
            display (bool, default=False): display original image and contours, area, and pixel area
        Return: 
    ...
    for file in sorted(os.listdir(directory)):
        ...
        if display == True: #show the original
            ...
            print('ORIGINAL')
            show(rimg)
    # Finding center color
        grayscaled = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        dataset = rasterio.open(directory+file)
        centerx,centery = (int(dataset.width/2),int(dataset.height/2))
        pixel_color = grayscaled[centery, centerx]
    #1. Binary threshold
        ret,thresh4 = cv2.threshold(grayscaled,pixel_color-10,pixel_color+5,cv2.THRESH_BINARY)
